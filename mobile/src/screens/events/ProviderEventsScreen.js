...
const ProviderEventsScreen = ({ navigation }) => {
  const { user } = useAuth();
  ...
  const fetchProviderEvents = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/events/provider/${user.userId}`);
      setEvents(response.data.events || []);
    } catch (error) {
      Alert.alert('Error', error.message || 'Failed to load provider events');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <ImageBackground
      ...
      blurRadius={1}
      accessibilityLabel="Provider events background"
    >
      <View style={styles.overlay}>
        <View style={styles.logoContainer} accessibilityLabel="Provider events logo">
          <Icon name="calendar-account" size={50} color="#6A5ACD" />
...
export default ProviderEventsScreen;