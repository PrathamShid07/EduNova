...
describe('API Integration Tests', () => {
    ...
    test('POST /api/events creates event for provider', async () => {
        const providerResponse = await request(app)
            .post('/api/auth/register')
            .send({ email: 'provider@example.com', password: 'password123', name: 'Provider', role: 'provider' });
        const providerToken = providerResponse.body.token;

        const response = await request(app)
            .post('/api/events')
            .set('Authorization', `Bearer ${providerToken}`)
            .send({ title: 'Test Event', description: 'Test', category: 'webinar', startDate: '2025-07-20T10:00:00Z', location: 'Online' });
    ...
    test('POST /api/providers creates provider profile', async () => {
        const providerResponse = await request(app)
            .post('/api/auth/register')
            .send({ email: 'provider2@example.com', password: 'password123', name: 'Provider2', role: 'provider' });
        const providerToken = providerResponse.body.token;

        const response = await request(app)
            .post('/api/providers')
            .set('Authorization', `Bearer ${providerToken}`)
            .send({ name: 'Test Provider', description: 'Test Description' });
    ...
    test('POST /api/contacts sends message to provider', async () => {
        const response = await request(app)
            .post('/api/contacts')
            .set('Authorization', `Bearer ${token}`)
            .send({ providerId, subject: 'Test Subject', message: 'Test Message' });
    ...
    test('POST /api/notifications/preferences updates preferences', async () => {
        const response = await request(app)
            ...
            .set('Authorization', `Bearer ${token}`)
            .send({ emailNotifications: true, pushNotifications: true, eventReminders: true });
